...
export default function ChatBot() {
  ...
  if (!isOpen) {
    return (
      <div className={`fixed left-4 z-50 transition-all duration-300 ${
        bannerVisible
          ? 'bottom-32 sm:bottom-28' // Higher when banner is visible
          : 'bottom-20 sm:bottom-24'  // Lower when banner is hidden
      }`}>
        <div className="relative">
          <Button
            onClick={() => setIsOpen(true)}
            className="bg-yellow-400 hover:bg-yellow-500 text-black rounded-full w-14 h-14 sm:w-16 sm:h-16 p-3 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-110 border-0 relative z-10"
  ...
  return (
    <Card className={`fixed left-4 z-50 flex flex-col shadow-xl border-2 border-yellow-400 transition-all duration-300 ${
      bannerVisible
        ? 'bottom-32 sm:bottom-28' // Higher when banner is visible
        : 'bottom-20 sm:bottom-24'  // Lower when banner is hidden
    } w-80 sm:w-96 h-80 sm:h-96`}>
      <CardHeader className="bg-yellow-400 text-black p-3 sm:p-4 rounded-t-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Bot className="w-4 h-4 sm:w-5 sm:h-5" />
            <span className="font-semibold text-sm sm:text-base">MMAC Assistant</span>
          </div>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setIsOpen(false)}
            className="text-black hover:bg-yellow-500 h-6 w-6 sm:h-8 sm:w-8 p-0"
          >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-2 sm:p-3 space-y-2 sm:space-y-3">
            ...
            >
              ...
              >
                <div className="flex items-start space-x-1 sm:space-x-2 w-full">
                  {message.isBot && <Bot className="w-3 h-3 sm:w-4 sm:h-4 mt-0.5 flex-shrink-0" />}
                  <div
                  ...
                  {!message.isBot && <User className="w-3 h-3 sm:w-4 sm:h-4 mt-0.5 flex-shrink-0" />}
                </div>
          ...
          {isTyping && (
            <div className="flex justify-start w-full">
              <div className="bg-gray-100 text-gray-900 p-2 sm:p-3 rounded-lg max-w-[75%]">
                <div className="flex items-center space-x-1 sm:space-x-2">
                  <Bot className="w-3 h-3 sm:w-4 sm:h-4" />
                  <div className="text-xs sm:text-sm">Typing...</div>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="border-t p-2 sm:p-3">
          <div className="flex space-x-2">
            ...
            <Button
              ...
              className="bg-yellow-400 hover:bg-yellow-500 text-black h-8 w-8 sm:h-10 sm:w-10 p-0"
            >
